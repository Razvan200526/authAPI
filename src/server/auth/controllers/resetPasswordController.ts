...
export class ResetPasswordController {
  ...
  async resetPassword(c: Context) {
    try {
      ...
      const sent = await this.emailService.sendPasswordRecoveryEmail(email, token);
      if (!sent) {
  ...
  async confirmReset(c: Context) {
    try {
      ...
      await this.db.update(users).set({ password: hashed }).where(eq(users.id, record.userId));
      await this.db.update(passwordResetTokens).set({ isUsed: 1 }).where(eq(passwordResetTokens.id, record.id));
